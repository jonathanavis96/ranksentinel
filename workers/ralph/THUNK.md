...
2026-01-29 14:56 | 4.5 | Daily critical email gating | Added _send_daily_critical_alerts() function to daily_checks.py that checks for critical findings and only sends email if they exist. Added compose_daily_critical_report() and render_daily_critical_alert() for critical-only email composition. Per-customer isolation ensures one failure doesn't affect others | AC: email gating implemented, critical-only composition ready
