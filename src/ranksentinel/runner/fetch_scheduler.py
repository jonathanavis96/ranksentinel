...
class FetchScheduler:
    ...
    def record_429(self, task: FetchTask) -> None:
        ...
        if (
            task.attempt < self.max_attempts_per_url
            and self.domain_429_count[domain] < self.max_429s_per_domain
        ):
            retry_task = FetchTask(
                customer_id=task.customer_id, url=task.url, attempt=task.attempt + 1
            )
    ...
    def get_domain_stats(self) -> dict[str, dict]:
        ...
        for domain in set(
            list(self.domain_429_count.keys()) + list(self.domain_next_allowed_at.keys())
        ):
            cooling_down = (
                domain in self.domain_next_allowed_at and now < self.domain_next_allowed_at[domain]
            )
            stats[domain] = {
