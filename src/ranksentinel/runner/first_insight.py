...
def run_first_insight_checks(
    conn: Connection, customer_id: int, run_id: str, psi_api_key: str | None = None
) -> None:
    ...
    """
    log_structured(
        run_id, run_type="first_insight", stage="init", status="start", customer_id=customer_id
    )
...
Please verify the sitemap URL is correct and accessible."""
    ...
    for t in targets:
        ...
        store_artifact(
            conn, customer_id, "meta_robots", url, curr_meta_sha, data["meta_robots"], fetched_at
        )

        curr_canonical_sha = sha256_text(data["canonical"])
        store_artifact(
            conn, customer_id, "canonical", url, curr_canonical_sha, data["canonical"], fetched_at
        )
        ...
        if psi_enabled and psi_count < psi_limit and psi_api_key:
            ...
            if psi_metrics:
                ...
                log_structured(
                    ...
                    status="success",
                    url=url,
                    perf_score=psi_metrics.get("perf_score"),
                )
