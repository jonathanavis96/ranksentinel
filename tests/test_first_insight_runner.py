...
from ranksentinel.runner.first_insight import run_first_insight_checks, trigger_first_insight_report
from ranksentinel.db import execute, fetch_all, fetch_one
...
def test_first_insight_creates_baseline_artifacts(customer_with_targets):
    ...
    execute(
        ...
        "VALUES(?, ?, ?, ?, ?, ?)",
        (customer_id, "robots_txt", "https://example.com", "abc123", "User-agent: *\nDisallow:", now),
    )
    ...
    execute(
        ...
        "INSERT INTO artifacts(customer_id, kind, subject, artifact_sha, raw_content, fetched_at) "
        "VALUES(?, ?, ?, ?, ?, ?)",
        (customer_id, "sitemap", "https://example.com/sitemap.xml", "def456", "<urlset></urlset>", now),
    )
