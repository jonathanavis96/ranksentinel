...
"""
    ...
    artifacts = fetch_all(
        conn,
        "SELECT kind, subject, artifact_sha, raw_content FROM artifacts WHERE customer_id=1"
    )
    
    assert len(artifacts) > 0, "No artifacts stored"
    
    robots_artifacts = [a for a in artifacts if a["kind"] == "robots_txt"]
    assert len(robots_artifacts) == 1, f"Expected 1 robots_txt artifact, got {len(robots_artifacts)}"
...
def test_robots_fetch_changed_content(test_db):
    ...
    artifacts = fetch_all(
        conn,
        "SELECT artifact_sha, raw_content FROM artifacts WHERE kind='robots_txt' ORDER BY id"
    )
...
def test_robots_fetch_error_handling(test_db):
    ...
    robots_artifacts = fetch_all(
        conn,
        "SELECT * FROM artifacts WHERE kind='robots_txt'"
    )
...
def test_robots_fetch_fallback_to_target_url(test_db):
    ...
    artifact = fetch_one(
        conn,
        "SELECT subject FROM artifacts WHERE kind='robots_txt'"
    )
