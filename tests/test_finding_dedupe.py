...
from ranksentinel.db import generate_finding_dedupe_key, init_db, execute
...
def test_generate_dedupe_key_uniqueness():
    ...
    base_key = generate_finding_dedupe_key(
        1, "daily", "indexability", "Title", "https://example.com", "2026-01-29"
    )

    # Different customer
    assert base_key != generate_finding_dedupe_key(
        2, "daily", "indexability", "Title", "https://example.com", "2026-01-29"
    )

    # Different run_type
    assert base_key != generate_finding_dedupe_key(
        1, "weekly", "indexability", "Title", "https://example.com", "2026-01-29"
    )

    # Different category
    assert base_key != generate_finding_dedupe_key(
        1, "daily", "performance", "Title", "https://example.com", "2026-01-29"
    )

    # Different title
    assert base_key != generate_finding_dedupe_key(
        1, "daily", "indexability", "Different Title", "https://example.com", "2026-01-29"
    )

    # Different URL
    assert base_key != generate_finding_dedupe_key(
        1, "daily", "indexability", "Title", "https://other.com", "2026-01-29"
    )

    # Different period
    assert base_key != generate_finding_dedupe_key(
        1, "daily", "indexability", "Title", "https://example.com", "2026-01-30"
    )
...
def test_finding_dedupe_prevents_duplicates(test_db):
    ...
    count = test_db.execute(
        "SELECT COUNT(*) FROM findings WHERE customer_id=?", (customer_id,)
    ).fetchone()[0]
